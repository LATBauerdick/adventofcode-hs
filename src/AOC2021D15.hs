...
aoc15 = do
  ...
  let ls = lines ss
      ...
      neighbors :: Int -> (Maybe Int, Maybe Int); neighbors i = (r,d) where
...
-- brute force works for the 10x10 example, but blows up for the larger case
      ...
      buildNode s i = let a = Unsafe.at i risks; ss = s+a
                       in if ss > nl*ll*3 then Leaf else
                            case neighbors i of
                              (Just r, Just d)   -> Node a (buildNode ss r) (buildNode ss d)
                              (Just r, Nothing)  -> Node a (buildNode ss r) Leaf
                              (Nothing, Just d)  -> Node a Leaf (buildNode ss d)
...
-- instead, draw one path and then vary the path only accepting less risky paths
--
